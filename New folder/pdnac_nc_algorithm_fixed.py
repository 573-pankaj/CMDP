...
FIXED BUGS:
- Proper handling of frozen output layer in critic
- Correct gradient computation for all parameters
- Proper MLMC implementation
"""
...
class PDNAC_NC:
    ...
    def compute_mlmc_critic_gradient(
        ...
        Returns:
            Dictionary of MLMC gradient estimates for each parameter
        """
        ...
        if P_kh > 0 and 2**P_kh <= self.config.T_max:
            if P_kh > 0:
                levels_to_compute.append(P_kh - 1)
        ...
        for level in levels_to_compute:
            ...
            for name in gradients:
                if gradients[name]:
                    ...
                    # Get shape from critic parameter
        ...
        if 0 in level_estimates:
            ...
            for name, param in critic.named_parameters():
                if param.requires_grad:
                    v_mlmc[name] = torch.zeros_like(param)
        ...
        if P_kh > 0 and 2**P_kh <= self.config.T_max and P_kh in level_estimates:
            for name in v_mlmc:
                ...
                diff = level_P - level_P_minus_1
                v_mlmc[name] = v_mlmc[name] + (2**P_kh) * diff
    ...
    def update_critic(
        ...
        # Only reset trainable parameters (not frozen output layer)
        with torch.no_grad():
            for name, param in critic.named_parameters():
        ...
        for h in range(self.config.H):
            # Compute MLMC gradient estimate (returns dict of gradients)
            v_mlmc_dict = self.compute_mlmc_critic_gradient(
                critic,
                objective,
                initial_state
            )
    ...
    def compute_fisher_matrix_sample(
        ...
        log_prob = action_dist.log_prob(action).sum()  # Sum for multi-dimensional actions
        ...
        for param in self.policy.parameters():
            if param.grad is not None:
                score.append(param.grad.view(-1))
    ...
    def compute_npg_gradient_sample(
        ...
        policy_gradient = advantage.item() * score_vector
...
if __name__ == "__main__":
    ...
    print("PDNAC-NC Algorithm Implementation (FIXED VERSION)")
    print("=" * 60)
